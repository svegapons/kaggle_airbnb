...
def clf_sklearn(clf, data, random_state=0, calibrated=False, clf_name='lr', ext_name=''):
    """
    ...
    #Calibration
    if calibrated:
        cc_clf = CalibratedClassifierCV(base_estimator=clf, method='isotonic', cv=3)
    ...
    ndcg = np.mean([ndcg_score(tr, pr, k=5) for tr, pr in zip(yb_valid.tolist(), y_valid_pred.tolist())])
    logloss = log_loss(y_valid, y_valid_pred)
    print 'Validation results with calibration, ndcg5: %s, logloss: %s' %(ndcg, logloss)        
    
    #Saving the result
    rnd = random_state.randint(1000, 9999)
    pickle.dump(y_valid_pred, open('save/valid/v_%s_%s_%s_%s_%s'%(clf_name, ext_name, rnd, round(ndcg, 4), round(logloss, 4)), 'w'))
